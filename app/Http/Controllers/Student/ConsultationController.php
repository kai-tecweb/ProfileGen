...
class ConsultationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $prompt .= "【クライアントの質問】\n{$question}\n\n";
            $prompt .= "【重要】回答は必ず以下の4段階形式で返してください：\n\n";
            $prompt .= "【要約】\n";
            $prompt .= "（2-3行で要点のみを簡潔にまとめる）\n\n";
            $prompt .= "【今すぐやること】\n";
            $prompt .= "（この回答を受けて、相談者が今すぐ実行すべき具体的なアクションを箇条書きで3-5個記載）\n";
            $prompt .= "（例：• ○○を確認する、• ××に登録する、• △△を作成する）\n\n";
            $prompt .= "【アドバイス】\n";
            $prompt .= "（心構え、注意点、コツ、成功のポイントなどを箇条書きで2-4個記載）\n";
            $prompt .= "（例：• まずは完璧を求めず行動しよう、• 困ったらすぐに相談してください）\n\n";
            $prompt .= "【詳細】\n";
            $prompt .= "（具体的な説明、背景、理由、ステップ、補足情報などを詳しく記載）\n\n";
